...
@Component({
    selector: 'app-optometrist-create',
    imports: [FormsModule, PageHeaderComponent, FormActionsComponent, OptometristFormComponent],
    ...
    template: `
        <div class="ren-page">
            @if (!exists()) {
                ...
                <app-page-header title="New Optometry Exam" icon="heroicons_outline:eye" />
                <form class="ren-card ren-card-body" (ngSubmit)="save()">
                    <app-optometrist-form [model]="model" />
                    <app-form-actions saveLabel="Save Exam" [cancelHref]="'/patient-dashboard/' + patientId" [saving]="saving()" />
                </form>
            }
        </div>
    `
})
export class OptometristCreateComponent implements OnInit {
    ...
    patientId = '';
    model: Optometrist = { id: '', patientId: '' };
    ...
    save(): void {
        ...
        this.api.createOptometrist(this.model).subscribe({
            next: () => {
                this.saving.set(false);
                this.notifications.showSuccess('Optometry exam saved.');
                if (this.model.referred) {
                    this.dialog.open(ReferralPromptDialogComponent, {
                        ...
                        data: {
                            ...
                            sourceModule: AppModule.Optometrists,
