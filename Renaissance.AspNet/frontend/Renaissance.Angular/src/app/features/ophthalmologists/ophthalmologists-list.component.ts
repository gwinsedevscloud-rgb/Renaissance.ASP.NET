...
import { Ophthalmologist, Patient } from '../../core/models/clinical.models';
...
@Component({
    selector: 'app-ophthalmologists-list',
    imports: [
    ...
    template: `
        <div class="ren-page">
            <app-page-header title="Ophthalmologists" subtitle="Ophthalmology care" icon="heroicons_outline:eye" />
            <app-referral-queue-panel [module]="AppModule.Ophthalmologists" />
            <app-patient-quick-search createHrefPrefix="/ophthalmologists/create" actionLabel="New record" />
            @if (loading()) {
                <app-loading-state message="Loading ophthalmology records..." />
            } @else if (items().length === 0) {
                <app-empty-state icon="heroicons_outline:eye" title="No ophthalmology records yet"
                    message="Search for a patient above to record ophthalmology care." />
            } @else {
                ...
                <div class="ren-card ren-table-card">
                    <table mat-table [dataSource]="items()" class="ren-table w-full">
                        ...
                        <ng-container matColumnDef="diagnoses">
                            ...
                            <td mat-cell *matCellDef="let o">{{ o.diagnoses?.[0] ?? '—' }}</td>
                        </ng-container>
                        ...
                        <ng-container matColumnDef="actions">
                            ...
                            <td mat-cell *matCellDef="let o">
                                <a mat-icon-button [routerLink]="['/ophthalmologists/edit', o.id]"><mat-icon svgIcon="heroicons_outline:pencil"></mat-icon></a>
                            </td>
                        </ng-container>
...
export class OphthalmologistsListComponent implements OnInit {
    private readonly api = inject(RenaissanceApiService);
    readonly AppModule = AppModule;

    readonly loading = signal(true);
    readonly items = signal<Ophthalmologist[]>([]);
    readonly patients = signal<Map<string, Patient>>(new Map());
    readonly columns = ['patient', 'diagnoses', 'va', 'actions'];

    ngOnInit(): void {
        forkJoin({ patients: this.api.getPatients(), items: this.api.getOphthalmologists() }).subscribe({
            next: ({ patients, items }) => {
